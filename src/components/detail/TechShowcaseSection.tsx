...
const showcaseData: Record<string, ShowcaseItem[]> = {
  ...
  'crossborder-workflow': [
    {
      title: '多平台 API 对接与同步引擎',
      text: 'CrossFlow 对接 5 个跨境电商平台，实现 4 种不同的 API 认证方案：Amazon SP-API 使用 OAuth2 授权码流程 + RDT 令牌刷新，分页遍历 11 个站点订单；Shopee Open API 采用 HMAC-SHA256 签名 + 公共参数 + 业务参数拼装，覆盖 9 个国家；Lazada Open Platform 使用签名请求接入 6 国站点；TikTok Shop Partner API 为主要链路（x-tts-access-token + shopCipher），Cookie 爬取 seller center 内部 API 为自动降级方案，支持 8 个站点。\n\n同步引擎基于 node-cron 调度，每平台独立间隔（默认 15 分钟）。Set 并发锁防止同平台重复同步——手动触发若检测到已有调度任务运行则跳过。令牌桶限流器统一管理 API 配额：Amazon 5 tokens/1 rps，Shopee 10/10。遇 429 响应自动清空令牌桶并 2 秒后重试。指数退避重试策略（1 min → 5 min → 15 min，最多 3 次），重试仍失败记录到 sync_log 并推送通知。',
      imageRight: true,
    ...
    {
      title: 'SKU 匹配引擎与智能合单',
      text: '订单从各平台同步后需要将平台 SKU 映射到本地 Product 记录。CrossFlow 实现了三策略匹配管线：Strategy 1——product_platform 关联表精确匹配（已有映射直接命中），Strategy 2——product.sku 直接匹配（Excel 导入等无关联表的场景），Strategy 3——去空格/连字符/下划线后大小写不敏感模糊匹配（兜底策略）。匹配成功后自动创建 product_platform 映射供下次精确命中。\n\n合单引擎在订单同步后自动检测：同 SKU + 同收货地址 + 待处理状态 = 候选合并订单。用户确认合并后触发二次校验（重新验证所有条件），将子单数量金额汇总到主单、reassign order_item 行、子单标记为已取消。上限 20 单/次，防止超大批量合并导致的性能问题。合并后自动扣减库存预留。',
      imageRight: false,
      code: `// SKU matching pipeline — 3 strategies\nfunction matchSku(platformSku: string): Product | null {\n  // Strategy 1: exact match via product_platform join\n  const exact = db.prepare(\`\n    SELECT p.* FROM product p\n    JOIN product_platform pp ON p.id = pp.product_id\n    WHERE pp.platform_sku = ?\n  \`).get(platformSku);\n  if (exact) return exact;\n\n  // Strategy 2: direct match on product.sku\n  const direct = db.prepare(\n    'SELECT * FROM product WHERE sku = ?'\n  ).get(platformSku);\n  if (direct) return direct;\n\n  // Strategy 3: fuzzy match (strip special chars)\n  const normalized = platformSku.replace(/[-_\\s]/g, '').toLowerCase();\n  const fuzzy = db.prepare(\`\n    SELECT * FROM product\n    WHERE REPLACE(REPLACE(REPLACE(LOWER(sku), '-', ''), '_', ''), ' ', '') = ?\n  \`).get(normalized);\n  return fuzzy || null;\n}`,
    },
    {
      title: '智能补货算法与利润计算',
      text: '库存模块实现三态模型（可用 available / 已预留 reserved / 在途 in_transit），多仓库独立跟踪。补货建议基于加权移动平均算法：最近 7 天日均销量权重 50% + 8-14 天权重 30% + 15-30 天权重 20%，乘以备货周期天数再乘以 1.2 安全系数，自动分级紧急（库存 < 3 天）、高（< 7 天）、正常（< 14 天）。补货数量校验防止负库存预留。全链路 audit_log 表记录每次库存变更的 before/after 值和操作来源。\n\nDashboard 利润计算器支持多币种营收转 USD：LEFT JOIN exchange_rate 表取最新汇率，减去采购成本、各平台佣金费率 × 营收额、支付处理费率 × 营收额 + 固定手续费。所有费用项通过 COALESCE 处理可选配置，避免 NULL 导致整行计算失败。利润数据按 SKU、平台、时间维度可下钻查看。',
      imageRight: true,
      code: `// Weighted moving average restock suggestion\nfunction getRestockSuggestions(): RestockSuggestion[] {\n  const W1 = 0.5, W2 = 0.3, W3 = 0.2;\n  const SAFETY = 1.2;\n\n  return db.prepare(\`\n    SELECT p.sku, p.name, inv.available, inv.reserved,\n      inv.safety_stock, s.lead_time,\n      COALESCE(d7.avg_daily, 0) * \${W1} +\n      COALESCE(d14.avg_daily, 0) * \${W2} +\n      COALESCE(d30.avg_daily, 0) * \${W3}\n      AS weighted_avg_daily\n    FROM inventory inv\n    JOIN product p ON inv.product_id = p.id\n    LEFT JOIN supplier s ON p.supplier_id = s.id\n    LEFT JOIN (...daily_sales_7d...) d7\n    LEFT JOIN (...daily_sales_14d...) d14\n    LEFT JOIN (...daily_sales_30d...) d30\n    WHERE inv.available + inv.in_transit <\n      weighted_avg_daily * s.lead_time * \${SAFETY}\n  \`).all();\n}`,
    },
    {
      title: '安全架构与生产可靠性',
      text: 'CrossFlow 从设计之初就考虑生产环境的安全需求。多用户认证：pbkdf2(100K iterations, SHA-512) 密码哈希 + 4 级 RBAC（管理员/运营/客服/仓管），登录限流 5 次/分钟防暴力破解，audit_log 记录登录事件。API 密钥安全：通过 Electron safeStorage 加密存储（Windows DPAPI / macOS Keychain / Linux libsecret），渲染进程通过 IPC 请求密钥时返回的是解密后的值但绝不明文存盘。旧版 electron-store 明文密钥自动迁移加密，迁移后删除原文。\n\n数据可靠性：sql.js 内存快照导出（db.export()）实现无锁备份，原子 renameSync 防止备份中断。PreparedStatement 包装器模拟同步 API 屏蔽 WASM 异步复杂性，写操作后 5 秒防抖自动存盘。CSV 导出带 UTF-8 BOM（Excel 兼容）和公式注入防护（=+-@ 前缀自动转义为 tab 前缀）。订单状态防降级：已发货订单重新同步不会回退到待处理。页面级 ErrorBoundary 防止白屏，侧边栏和 Header 始终可用。',
      imageRight: false,
      code: `// Safe column mapping — SQL injection prevention\nconst SAFE_COLUMNS: Record<string, Set<string>> = {\n  product: new Set(['name', 'name_en', 'sku', 'category', 'cost_price', 'weight']),\n  order: new Set(['status', 'platform_status', 'tracking_number', 'shipped_time']),\n  inventory: new Set(['available', 'reserved', 'in_transit', 'safety_stock']),\n};\n\nfunction buildUpdateSQL(table: string, id: number, data: Record<string, unknown>): string {\n  const cols = SAFE_COLUMNS[table];\n  const sets = Object.entries(data)\n    .filter(([k]) => cols.has(k))\n    .map(([k, v]) => \`\${k} = \${escapeValue(v)}\`)\n    .join(', ');\n  return \`UPDATE \${table} SET \${sets} WHERE id = \${id}\`;\n}`,
    },
  ],
  'russiablock': [
    {
